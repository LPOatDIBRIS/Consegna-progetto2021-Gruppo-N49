...
import static Lpocej.visitors.typechecking.PrimType.*;
...
public class TypeCheck implements Visitor<Type> {
	...
	@Override
	public PrimType visitNotEq(Exp left, Exp right) {							//NEW ADD
		left.accept(this).checkEqual(right.accept(this));
		return BOOL;
	}

	@Override
	 public PrimType visitRange(Exp left, Exp right) {							//NEW ADD
		checkBinOp(left, right, INT);
		return RANGE;
	}  
	
	@Override
	public ProdType visitBound(Exp exp){										// NEW ADD
		RANGE.checkEqual(exp.accept(this));
		return new ProdType(INT, INT);
	}
	
	@Override     
	public Type visitForStmt(VarIdent ident, Exp exp, Block thenBlock){			//NEW ADD
		RANGE.checkEqual(exp.accept(this));
		env.enterScope();	
		env.dec(ident, INT); 
		thenBlock.accept(this);
